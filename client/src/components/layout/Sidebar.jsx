...
import { 
  ...
  Building2, Search, MessageSquare, Package, 
  Settings, LogOut, ChevronLeft, ChevronRight,
  Pill, Truck, RefreshCw, HandHeart, Shield, Megaphone
} from 'lucide-react';
...
export function Sidebar({ mobileOpen, onMobileClose }) {
  ...
  const activeSOS = sosRequests?.length || 0;
  const role = user?.role;

  // Common: shown to ALL authenticated users
  const commonItems = [
  ...
  // Civilian-only
  const civilianOnlyItems = [
    { icon: <AlertTriangle size={20} />, label: 'Emergency Alert', href: '/sos', badge: activeSOS },
    { icon: <Users size={20} />, label: 'Family Circle', href: '/family' },
    { icon: <Pill size={20} />, label: 'Medicine', href: '/medicine' },
    { icon: <Truck size={20} />, label: 'Transport', href: '/transport' },
    { icon: <HandHeart size={20} />, label: 'Volunteer', href: '/volunteer' },
    { icon: <FileText size={20} />, label: 'Documents', href: '/documents' },
    { icon: <AlertTriangle size={20} />, label: 'Hazard Report', href: '/hazard-report' },
  ];

  // Agency-only
  const agencyOnlyItems = [
    { icon: <Shield size={20} />, label: 'SOS Command', href: '/sos', badge: activeSOS },
    { icon: <Megaphone size={20} />, label: 'Programs & Alerts', href: '/agency-services' },
    { icon: <Package size={20} />, label: 'Inventory', href: '/logistics' },
    { icon: <RefreshCw size={20} />, label: 'Resource Exchange', href: '/exchange' },
    { icon: <MessageSquare size={20} />, label: 'Agency Chat', href: '/agency-chat' },
    { icon: <Pill size={20} />, label: 'Medicine', href: '/medicine' },
    { icon: <Truck size={20} />, label: 'Transport', href: '/transport' },
    { icon: <HandHeart size={20} />, label: 'Volunteers', href: '/volunteer' },
    { icon: <AlertTriangle size={20} />, label: 'Hazard Reports', href: '/hazard-report' },
  ];

  // Admin — gets agency tools too
  ...
  if (role === 'civilian') navItems.push(...civilianOnlyItems);
  else if (role === 'agency') navItems.push(...agencyOnlyItems);
  else if (role === 'admin') navItems.push(...adminOnlyItems);
  navItems.push({ icon: <Settings size={20} />, label: 'Settings', href: '/profile' });
      ...
      <aside className={clsx(
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900 space-y-2">
          ...
          <button onClick={logout}
            className={clsx("flex items-center gap-3 px-2 py-2 w-full text-slate-400 hover:text-red-400 hover:bg-red-900/10 rounded-lg transition-colors", !sidebarOpen && "justify-center")}>
            <LogOut size={20} />
